"""Training script for det assertion 'G_IS_FILE (file1ector."""
import argparse
import copy
import os
...
def compute_loss(
    ...
    loss_test = criterion(prediction_batch[:, 5:, :, :],
        target_batch[:, 5:,:, :]) 
    ...
    return reg_mse, pos_mse, neg_mse, loss_test
...
def train(device: str = "cpu") -> None:
    ...
    dataset = CocoDetection(
        ...
        root="./Set1/training",
        annFile="./Set1/annotations/merged_training.json",
    ...
    while current_iteration <= NUM_ITERATIONS:
        for img_batch, target_batch in dataloader:
            ...
            reg_mse, pos_mse, neg_mse, loss_test = compute_loss(out, target_batch)
            loss = WEIGHT_POS * pos_mse + WEIGHT_REG * reg_mse + WEIGHT_NEG * neg_mse + WEIGHT_CLASS * loss_test
...
def validate(
    ...
    with torch.no_grad():
        ...
        for val_img_batch, val_target_batch in val_dataloader:
            ...
            reg_mse, pos_mse, neg_mse, loss_test = compute_loss(val_out, val_target_batch)
